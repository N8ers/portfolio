export default {
  projects: [
    {
      name: 'Dashboard',
      description: 'A landing page to organize your day',
      github: 'https://github.com/N8ers/dashboard',
      site: 'https://dashboard-299215.web.app/welcome',
      image: 'dashboard.png',
    },
    {
      name: 'Brewster',
      description: 'Find local breweries, no matter where life takes you',
      github: 'https://github.com/N8ers/brewster',
      site: 'https://brewster-c62ae.firebaseapp.com/',
      image: 'dashboard.png',
    },
    {
      name: 'Chat App',
      description: 'Chat with your friends using this real time chat app',
      github: '',
      site: '',
      image: 'dashboard.png',
    },
    {
      name: 'HTMLgenerator',
      description: 'Use a simple text editor to generate styled HTML',
      github: '',
      site: '',
      image: 'dashboard.png',
    },
    {
      name: 'Browser eq',
      description: 'Import an audio file, and make some custom eq',
      github: '',
      site: '',
      image: 'dashboard.png',
    },
    {
      name: 'Browser image cropper',
      description: 'Import an image and crop it how you please!',
      github: '',
      site: '',
      image: 'dashboard.png',
    },
    {
      name: 'Budgeter',
      description: 'A budgeting app for modern times',
      github: '',
      site: '',
      image: 'dashboard.png',
    },
  ],
  blogs: [
    {
      name: 'Enough to Git you started',
      description: 'A beginners guide to using Git',
      url: 'https://medium.com/@nathanraysheryak/enough-to-git-you-started-32d941730cb7',
    },
    {
      name: 'How to build your own NPM package',
      description: 'A Guide to publishing an npm package of your own',
      url: 'https://medium.com/@nathanraysheryak/enough-to-git-you-started-32d941730cb7',
    },
    {
      name: 'Adding Scss to an existing Vue project',
      description: '(the easy way)',
      url: '',
    },
    {
      name: 'A beginners guide to using GitHub actions',
      description: '',
      url: '',
    },
    {
      name: 'atomic css',
      description: 'what it is & why you should be using it',
      url: '',
    },
  ],
};
